...
describe('For', () => {
  describe('Liquid target', () => {
    it('should render Liquid for loop', () => {
      ...
      expect(result).toBe('{% for product in collection.products %}<div>{{ product.title }}</div>{% endfor %}');
    });
    ...
    it('should handle nested properties', () => {
      ...
      expect(result).toBe('{% for p in shop.collections.all.products %}<li>{{ p.price }}</li>{% endfor %}');
    });
    ...
    it('should work with literal arrays', () => {
      ...
      expect(result).toBe('{% for letter in ["a","b","c"] %}<span>{{ letter }}</span>{% endfor %}');
    });
  });
  ...
  describe('Edge cases', () => {
    it('should handle special characters in loop variable name', () => {
      ...
      expect(result).toBe('{% for my_item in items %}<div>{{ my_item.name }}</div>{% endfor %}');
    });
